...
class VideosController {
  async Create(req, res) {
    try {
      ...
      const archivesID = await createArchiveHelper({ thumbFile, videoFile: file, name: safeTitle });

      const languages = Array.isArray(language) ? language : [language];
      const languageData = await LanguageModel.find({ _id: { $in: languages } });

      if (languageData.length !== languages.length) {
        await fs.promises.unlink(tempPath).catch(console.error);
        return res.status(400).json({ message: "Invalid language ID(s)" });
      }

      const langValues = languageData.map(l => l.code || l.name);
      ...
      if (!transcriptionResult.pdfS3Key || !transcriptionResult.vttS3Key) {
        throw new Error("Error generating transcription or subtitles");
      }
